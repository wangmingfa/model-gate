...
- **administrator（管理员）**：操作 Web 配置界面的人，与 consumer（使用方/agent）是两类主体。本机回环访问免登录；非回环访问需密码登录（见 admin password），不参与 agent 的 keys 鉴权。
- **admin password（管理密码）**：config.json 顶层的 `admin_password` 字段（支持 `${ENV}` 插值），非回环访问管理界面时的登录密码；留空表示未配置（此时非回环访问时登录页提示去配置文件配置）。不进 GET/PUT config 的编辑范围，只在配置文件改。
- **admin session（管理会话）**：密码登录成功后服务端签发的内存会话 token（24 小时过期，重启失效），以 cookie 形式随请求携带；`/api/auth-status` 返回 `{ passwordConfigured, configPath, loggedIn }` 供登录页三态渲染。
