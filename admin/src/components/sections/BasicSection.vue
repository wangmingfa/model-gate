...
import { NCard, NForm, NFormItem, NSelect, NSpace, NStatistic, NIcon } from 'naive-ui';
  ...
  <n-card size="small" class="soft-card">
    ...
    <n-form-item label="默认模型（agent 未指定 model 时使用）" style="margin-top: 12px">
      <div :class="{ 'field-error': store.defaultModelError }" style="width: 100%">
        <n-select v-model:value="store.defaultModel" :options="store.defaultModelOptions" placeholder="选择一个别名" @update:value="store.autoSave()" />
      </div>
    </n-form-item>
  </n-card>
</template>
